export default async function getMediaFields(description){
    ...
    try {
      let config = {
        ...
        let res = await fetch(`http://127.0.0.1:8000/Media-Field/get_media_field_view/?media__description=${description}`, config)
        data = await res.json()
        console.log(data)
       /* if(res.status == 401){
          console.log("401")
