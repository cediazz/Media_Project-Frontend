export default async function getMedias(page=1,description="",category="",plan=""){
    ...
    try {
      let config = {
        ...
        let res = await fetch(`http://127.0.0.1:8000/Media/get_media_view/?category__description=${category}&description=${description}&page=${page}&plan__description=${plan}`, config)
        data = await res.json()
        console.log(data)
       /* if(res.status == 401){
          console.log("401")
