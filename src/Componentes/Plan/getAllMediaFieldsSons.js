export default async function getAllMediaFieldsSons(description="",category="",plan=""){
    ...
    try {
      let config = {
        ...
        let res = await fetch(`http://127.0.0.1:8000/Media/get_media_fields_sons/?description=${description}&category__description=${category}&plan__description=${plan}`, config)
        data = await res.json()
        console.log(data)
       /* if(res.status == 401){
          console.log("401")
